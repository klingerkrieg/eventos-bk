...
class GTS extends CI_Controller {
    ...
    public function index($id=null){
        ...
        if ($id == null){
            ...
            $lista = $this->GTS_model->listar($_GET);
        } else {
            if ($id == "form"){
                $dados = [];
            } else {
                $dados = $this->GTS_model->get($id);
            }
        ...
        include view("admin/gts");
    }


    public function salvar(){

        $this->form_validation->set_rules('gt', 'Nome do GT', 'required');

        if ($this->form_validation->run()) {
            #valido
            $this->load->model("GTS_model");
            $id = $this->GTS_model->salvar($this->input->post());
            if ($id == null){
                $this->session->set_flashdata("error","Não foi possível salvar o registro.");
            } else {
                $this->session->set_flashdata("success","Registro salvo.");
            } 
            redirect("painel/gts/index/$id");
        } else {
            #invalido
            $this->index("form");
        }
